...
function initRTC(opts) {
    ...
    RTC.createRoom({
        ...
        gotStream({
            ...
            $.ajax({
        		type: "POST",
        		url: 'https://223.3.72.234:8080/webrtc/mixStream',
        		dataType: 'json',
...
function login(closeLocalMedia) {
    ...
    $.ajax({
        type: "POST",
        url: 'https://223.3.72.234:8080/webrtc/testRTC',
        dataType: 'json',
		data:{roomid:parseInt($("#roomid").val()),userid:userId},
        success: function (json) {
            if (json && json.errorCode === 0) {
                //一会儿进入房间要用到
                var userSig = json.userSig;
                var privateMapKey = json.privMapEncrypt;
                var sdkappid=json.sdkappid;
                // 页面处理，显示视频流页面
                $("#video-section").show();
...
function startLiveTape(){
    ...
    $.ajax({
        type: "POST",
        url: 'http://223.3.72.234:8080/webrtc/startLiveTape',
        dataType: 'json',
		data:{roomid:parseInt($("#roomid").val()),userid:userId},
        success: function (json) {
            if (json && json.errorCode === 0) {
